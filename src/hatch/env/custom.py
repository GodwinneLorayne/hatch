    ...
    def __new__(  # type: ignore
        ...
        **kwargs: Any,
        ...
        if not isinstance(custom_script, str):
            message = f'Option `path` for environment `{cls.PLUGIN_NAME}` must be a string'
            raise TypeError(message)

        if not custom_script:
            message = f'Option `path` for environment `{cls.PLUGIN_NAME}` must not be empty if defined'
        ...
        hook_class = load_plugin_from_script(
            path, custom_script, EnvironmentInterface, 'environment'
        )
